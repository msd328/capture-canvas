...
3. Connect the one-time native callback grant to an atomic PKCE verifier/nonce handoff, then implement the bounded HTTPS token exchange, strict ID-token algorithm/signature/issuer/audience/expiry/nonce validation, refresh rotation, logout and revocation.
...
| HLT-29 | 🟡 | Native OIDC browser, loopback listener, callback validation, code expiry and one-time grant-take health | Path/token-free stages report bind, listen, browser launch, callback result, state rejection, invalid-attempt limit, timeout, cancellation, code clearing and grant-take success/missing/expired outcomes; unit replay test added and configured Windows validation pending |
| HLT-30 | 🟡 | Pinned OIDC token endpoint, issuer, audience and JWKS readiness health | `AuthHealth` reports only configured booleans and typed failure codes; endpoint/audience values remain native and Windows compile/configured-build validation is pending |
...
| SEC-12 | 🟡 | OS secure storage for future account tokens | Windows Credential Manager status/probe/clear is present; the PKCE verifier and temporary authorization code stay native-only, the callback grant is now one-time/replay-safe with a 60-second lifetime, and real sign-in remains gated on the complete pinned trust contract; Windows runtime, coordinated PKCE handoff, refresh-token write/rotation and macOS Keychain support remain |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, compile-time-pinned authorization/token trust metadata, browser/loopback capture, strict authorization-code/refresh form builders, strict bounded token-response parsing and a one-time native callback-grant take are implemented; Supabase provider registration, coordinated PKCE handoff, HTTPS exchange and ID-token validation remain |
