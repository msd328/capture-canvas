...
1. Pull the committed Rust lockfile and native signature-verifier batch, regenerate the lockfile once to add the explicit root `ring 0.17.14` dependency metadata, then run `scripts/windows-local-check.ps1` and `scripts/supabase-local-check.ps1` and resolve any TypeScript, Rust, migration, RLS, RPC, dependency-lock or pgTAP failures.
...
3. Wire the private bounded authorization-code exchange through exact JWKS key selection, maintained RS256/ES256 signature verification and strict issuer/client-audience/subject/time/nonce validation. Only a fully verified native identity may create an in-memory access-token session or rotate a refresh token in OS secure storage.
...
| HLT-30 | 🟡 | Pinned OIDC token endpoint, bounded transport, JWKS resolution, RS256/ES256 signature and ID-token claim health | `AuthHealth` reports only configured booleans, deadlines, cache hit/refresh, key counts, approved algorithm, signature/claim-validation booleans and typed failures for bounded token/JWKS transport, strict key/header/signature parsing, exact `kid`/algorithm selection, original `header.payload` verification and issuer/client-audience/canonical-subject/time/nonce checks; identifiers and token values remain native and Windows/configured-provider validation is pending |
...
| REL-14 | 🟡 | Windows CI build/test gate | Complete Windows local validation passed on 2026-07-31 through frozen Bun install, lint, production frontend build, Rust formatting, tests and Windows cargo check; a reviewed Cargo lockfile is now committed, while regeneration for the explicit native verifier dependency, the current clean-checkout locked run and the first green hosted run remain pending |
...
| SEC-12 | 🟡 | OS secure storage for future account tokens | Windows Credential Manager readiness/status/clear is present; the state-bound callback code/PKCE/nonce handoff, bounded token response, pinned JWKS cache, exact RS256/ES256 key selection, maintained signature verification and strict issuer/client-audience/canonical-subject/time/nonce validation remain native-only, non-serializable and unpersisted; live exchange, in-memory access-token session, refresh-token write/rotation, logout/revocation and macOS Keychain support remain |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, pinned authorization/token/JWKS trust metadata, browser/loopback capture, strict bounded exchange contracts, coordinated one-time code/PKCE/nonce handoff, private bounded Rustls token transport, five-minute JWKS cache, forced key-rotation refresh, strict RS256/ES256 key/header selection, maintained original-signing-input signature verification and strict issuer/client-audience/canonical-subject/time/nonce claim validation are implemented; Supabase provider registration, live verified exchange, native access-token session, refresh rotation and logout/revocation remain |
