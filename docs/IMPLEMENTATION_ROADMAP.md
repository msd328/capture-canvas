...
3. Use the coordinated one-time native authorization-code/PKCE/nonce handoff to implement the bounded HTTPS token exchange, then enforce strict ID-token algorithm/signature/issuer/audience/expiry/nonce validation before refresh rotation, logout and revocation.
...
| SEC-12 | 🟡 | OS secure storage for future account tokens | Windows Credential Manager readiness/status/clear is present; the callback authorization code, matching PKCE verifier and nonce now have a coordinated state-bound one-time native handoff with expiry and best-effort zeroing; Windows validation, HTTPS exchange, refresh-token write/rotation and macOS Keychain support remain |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, pinned authorization/token trust metadata, browser/loopback capture, strict bounded exchange contracts and a coordinated state-matched one-time authorization-code/PKCE/nonce handoff are implemented; Supabase provider registration, HTTPS token exchange and ID-token validation remain |
