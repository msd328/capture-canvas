...
3. Use the bounded JWKS resolver, exact `kid`/algorithm selection and strict issuer/client-audience/subject/time/nonce claims boundary to implement maintained RS256/ES256 signature verification. Only after signature acceptance may the native flow enable live exchange, refresh rotation, logout and revocation.
...
| HLT-30 | 🟡 | Pinned OIDC token endpoint, issuer, audience, bounded transport, JWKS resolution and ID-token claim health | `AuthHealth` reports only configured booleans, deadlines, cache hit/refresh, key counts, approved algorithm, claim-validation booleans and typed failures for bounded token/JWKS transport, strict key/header parsing, exact `kid`/algorithm selection and issuer/client-audience/canonical-subject/time/nonce checks; identifiers and token values remain native and Windows/configured-provider validation is pending |
...
| SEC-12 | 🟡 | OS secure storage for future account tokens | Windows Credential Manager readiness/status/clear is present; the state-bound callback code/PKCE/nonce handoff, bounded unverified token response, pinned JWKS cache, exact RS256/ES256 `kid`/algorithm selection and strict issuer/client-audience/canonical-subject/time/nonce payload validation remain native-only, non-serializable and unpersisted; live exchange stays gated on cryptographic signature verification, and Windows validation, refresh-token write/rotation and macOS Keychain support remain |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, pinned authorization/token/JWKS trust metadata, browser/loopback capture, strict bounded exchange contracts, coordinated one-time code/PKCE/nonce handoff, private bounded Rustls token transport, five-minute JWKS cache, forced key-rotation refresh, strict RS256/ES256 key/header selection and strict issuer/client-audience/canonical-subject/time/nonce claim validation are implemented; Supabase provider registration, live exchange, cryptographic signature verification and session creation remain |
