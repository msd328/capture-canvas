...
1. Pull the committed lockfile, live-completion and atomic refresh/session-commit batches, run `scripts/windows-local-check.ps1` and `scripts/supabase-local-check.ps1`, and resolve any TypeScript, Rust, migration, RLS, RPC, dependency-lock or pgTAP failures.
...
3. Validate the no-argument native authorization-code completion path against a configured Supabase provider, including exact JWKS key resolution, RS256/ES256 verification, strict identity/nonce checks, one-time replay rejection, transactional Windows refresh persistence, clear-during-network cancellation and native session expiry. Then implement refresh rotation/restoration, startup reconciliation and logout/revocation.
...
| HLT-26 | 🟡 | Native secure-auth storage status, readiness and live refresh-transaction health | `AuthHealth` emits only stage, support, status, result, replacement/read-back/rollback and session-commit booleans plus numeric Windows error codes; the live credential/session transaction is implemented and Windows/configured-provider evidence remains pending |
