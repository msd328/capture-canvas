...
1. Pull the committed lockfile and native live-completion batch, run `scripts/windows-local-check.ps1` and `scripts/supabase-local-check.ps1`, and resolve any TypeScript, Rust, migration, RLS, RPC, dependency-lock or pgTAP failures.
...
3. Validate the no-argument native authorization-code completion path against a configured Supabase provider, including exact JWKS key resolution, RS256/ES256 verification, strict identity/nonce checks, one-time replay rejection and in-memory session expiry. Then implement refresh-token persistence, replacement rollback, rotation, restoration and logout/revocation.
...
| HLT-30 | 🟡 | Pinned OIDC token endpoint, bounded transport, JWKS resolution, RS256/ES256 signature, identity validation and native-session health | `AuthHealth` reports only configured booleans, deadlines, cache hit/refresh, key counts, approved algorithm, signature/claim-validation booleans, no-WebView-token-input completion stages and native-only session/persistence booleans; identifiers and token values remain native and Windows/configured-provider validation is pending |
...
| REL-14 | 🟡 | Windows CI build/test gate | Complete Windows local validation passed on 2026-07-31 through frozen Bun install, lint, production frontend build, Rust formatting, tests and Windows cargo check; the reviewed Cargo lockfile now includes the explicit root `ring 0.17.14` edge, while the current live-completion clean-checkout locked run and first green hosted run remain pending |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, pinned authorization/token/JWKS trust metadata, browser/loopback capture, strict bounded exchange contracts, coordinated one-time code/PKCE/nonce handoff, bounded Rustls transport, five-minute JWKS cache, forced key-rotation refresh, exact RS256/ES256 key/header selection, maintained original-signing-input verification, strict identity/nonce validation, a no-argument completion command, automatic Settings completion and a native-only in-memory access session are implemented; configured Supabase runtime validation, refresh persistence/rotation/restoration and logout/revocation remain |
| SAAS-02 | 🟡 | Windows Credential Manager readiness/status/clear, transient native-only PKCE handling, verified-session status and failure-isolated Settings checks are implemented; Windows validation and real refresh-token write/replacement/rotation remain |
