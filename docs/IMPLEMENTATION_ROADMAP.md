...
1. Pull the committed lockfile, live-completion and staged refresh-transaction batches, run `scripts/windows-local-check.ps1` and `scripts/supabase-local-check.ps1`, and resolve any TypeScript, Rust, migration, RLS, RPC, dependency-lock or pgTAP failures.
...
3. Validate the no-argument native authorization-code completion path against a configured Supabase provider, including exact JWKS key resolution, RS256/ES256 verification, strict identity/nonce checks, one-time replay rejection and in-memory session expiry. The transactional Windows refresh-credential write/read-back/rollback primitive is implemented but not live; next wire it only after candidate validation, then implement rotation, restoration and logout/revocation.
...
| HLT-26 | 🟡 | Native secure-auth storage status, readiness and staged refresh-transaction health | `AuthHealth` emits only stage, support, status, result, replacement/read-back/rollback booleans and numeric Windows error codes; the transactional refresh write primitive is not live and Windows compile/probe evidence remains pending |
...
| HLT-30 | 🟡 | Pinned OIDC token endpoint, bounded transport, JWKS resolution, RS256/ES256 signature, identity validation and native-session health | `AuthHealth` reports only configured booleans, deadlines, cache hit/refresh, key counts, approved algorithm, signature/claim-validation booleans, no-WebView-token-input completion stages, native-only session state and staged refresh replacement/read-back/rollback outcomes; identifiers and token values remain native and Windows/configured-provider validation is pending |
...
| SEC-12 | 🟡 | OS secure storage for future account tokens | Windows Credential Manager readiness/status/clear, the state-bound callback code/PKCE/nonce handoff, bounded live token exchange, pinned JWKS cache, exact RS256/ES256 key selection, maintained signature verification, strict identity/nonce validation, a no-argument WebView completion surface, a native-only in-memory access session and a staged transactional refresh write/read-back/rollback primitive are implemented; configured-provider/Windows validation, live refresh persistence, rotation/restoration, logout/revocation, low-level store consolidation and macOS Keychain support remain |
...
| ID | Status | Work |
|---|---:|---|
| SAAS-01 | 🟡 | Native state/nonce/PKCE, pinned authorization/token/JWKS trust metadata, browser/loopback capture, strict bounded exchange contracts, coordinated one-time code/PKCE/nonce handoff, bounded Rustls transport, five-minute JWKS cache, forced key-rotation refresh, exact RS256/ES256 key/header selection, maintained original-signing-input verification, strict identity/nonce validation, a no-argument completion command, automatic Settings completion, a native-only in-memory access session and a staged transactional refresh-credential primitive are implemented; configured Supabase runtime validation, live refresh persistence/rotation/restoration and logout/revocation remain |
| SAAS-02 | 🟡 | Windows Credential Manager readiness/status/clear, transient native-only PKCE handling, verified-session status, failure-isolated Settings checks and a staged write/read-back/rollback replacement primitive are implemented; Windows validation, live refresh-token persistence, rotation/restoration and low-level store consolidation remain |
...
| SAAS-17 | 🟡 | Authoritative `/me/access` endpoint | Bounded RS256/ES256 JWKS verification, exact token claims, verified `sub`, parameterless `get_my_access()`, normalized access response and cross-user negative tests are implemented; real-token/Supabase runtime validation remains |
...
| BILL-15 | 🟡 | `/api/v1/me/access` | Verified JWT `sub` receives normalized provider/status/expiry/features through an owner-derived RPC; real Supabase token, JWKS rotation and runtime cross-user evidence remain pending |
